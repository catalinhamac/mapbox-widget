...
import { clustersLayerId, storesSourceId, unclusteredPointLayerId } from './Map';
...
export const onMapClick = (map: mapboxgl.Map) => {
  map.on(
    ...
    (
      ...
      (map.getSource(storesSourceId) as any).getClusterExpansionZoom(features[0].properties?.cluster_id,
        (err: any, zoom: number) => {
        if (err) return;
  ...
  map.on('click', unclusteredPointLayerId, (
    e: mapboxgl.MapMouseEvent & {
      features?: mapboxgl.MapboxGeoJSONFeature[] | undefined;
    } & mapboxgl.EventData
  ) => {
